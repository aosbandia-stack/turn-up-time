...
def validate_transition(project: Path, stage: str, errors: list[str]) -> None:
    ...
    if ledger.get("project_id") != project.name:
        errors.append(
            f"LEDGER project_id={ledger.get('project_id')} does not match directory {project.name}"
        )
    if ledger.get("spawn_budget", {}).get("used", 0) > ledger.get("spawn_budget", {}).get(
        "limit", 0
    ):
    ...
    if target_index >= order.index("DISCOVERY"):
        intake = artifact_status(
            project, "intake-readiness.json", "intake-readiness.schema.json", errors
        )
    ...
    if target_index >= order.index("EVIDENCE_REVIEW"):
        ...
        expected += (
            ["combined-engineering.json"]
            if profile == "lite"
            else ["frontend.json", "backend.json", "security.json"]
        )
        for filename in expected:
            artifact_status(
                project, f"evidence/{filename}", "evidence-pack.schema.json", errors
            )
    ...
    if target_index >= order.index("TICKETING"):
        definition = artifact_status(
            ...
            "definition-of-good.schema.json",
            errors,
        )
    ...
    if target_index >= order.index("BUILD"):
        seam = artifact_status(
            ...
            "seam-verdict.schema.json",
            errors,
        )
        if seam and (
            seam.get("phase") != "PRE_BUILD" or seam.get("status") != "SEAMS_SOUND"
        ):
